...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            $stmt = $pdo->prepare("
                ...
                SET leeftijd = :leeftijd, soort_lid_id = :soort_lid_id, 
                    basisbedrag = :basisbedrag, kortingspercentage = :kortingspercentage, boekjaar_id = :boekjaar_id
                WHERE id = :id
            ");
            $stmt->execute([
                'leeftijd' => $leeftijd,
    ...
    $_POST['basisbedrag'] = $contributie['basisbedrag'];
    $_POST['kortingspercentage'] = $contributie['kortingspercentage'];
...
<form method="POST" action="">
    ...
    <div class="form-group">
        ...
        <input type="number" id="basisbedrag" name="basisbedrag" value="<?php echo e($_POST['basisbedrag']); ?>" min="0" step="0.01" required readonly style="background-color: #f0f0f0;">
        <small>Het volledige contributietarief (standaard €100)</small>
    </div>
    
    <div class="form-group">
        <label for="kortingspercentage">Kortingspercentage (%):</label>
        <input type="number" id="kortingspercentage" name="kortingspercentage" value="<?php echo e($_POST['kortingspercentage']); ?>" min="0" max="100" step="0.01" required readonly style="background-color: #f0f0f0;">
